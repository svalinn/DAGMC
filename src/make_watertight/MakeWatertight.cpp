...
moab::ErrorCode MakeWatertight::seal_arc_pair(
  ...
  while (true) {
    ...
    if (e_dist > s_dist) {
      ...
      dist = s_dist;
      if (debug)
        std::cout << "skin is next, projected dist=" << dist << std::endl;
    } else {
      edge_is_next = true;
      dist = e_dist;
...
moab::ErrorCode MakeWatertight::seal_loop(
  ...
  // to a skin point.
  if (debug)
    std::cout << "seal_loop: new loop contains " << skin_loop.size()
              << " skin pts" << std::endl;
...
moab::ErrorCode MakeWatertight::prepare_surfaces(
  ...
  for (moab::Range::iterator i = surface_sets.begin(); i != surface_sets.end();
    ...
    result =
        seal_surface_loops(*i, &skin_loop_sets[0], skin, curve_sets, normal_tag,
...
moab::ErrorCode MakeWatertight::fix_normals(moab::Range surface_sets,
  ...
  moab::ErrorCode result;
  if (debug)
    std::cout << "number of surfaces=" << surface_sets.size() << std::endl;
...
moab::ErrorCode MakeWatertight::make_mesh_watertight(
  ...
  if (verbose)
    std::cout << "Getting entity count before sealing..." << std::endl;
