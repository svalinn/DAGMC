...
moab::ErrorCode write_sealed_file(moab::Interface* mbi,
                                  std::string output_file);

int main(int argc, char* argv[]) {
  ProgOptions po(
      "make_watertight: a tool for preprocessing DAGMC files to seal a faceted "
      "h5m file");
  ...
  po.addOpt<std::string>(
      "output_file,o",
      "Specify the output filename (default watertight_dagmc.h5m)",
      &output_file);

  po.parseCommandLine(argc, argv);

  if (output_file == "") output_file = "watertight_dagmc.h5m";
  ...
  std::cout
      << "To perform a tolerance based check run the check_Watertight tool"
      << std::endl;
  ...
  std::cout << "Timing(seconds): loading="
            ...
            << ", sealing=" << (double)(seal_time - load_time) / CLOCKS_PER_SEC
            << ", writing=" << (double)(write_time - seal_time) / CLOCKS_PER_SEC
            << ", checking="
