...
moab::ErrorCode MakeWatertightTest::check_num_ents(int ent_dimension,
                                                   int expected_num) {
  ...
  result = MBI()->get_entities_by_type_and_tag(
      0, moab::MBENTITYSET, &geom_tag, &val_ptr, 1, entities,
      moab::Interface::INTERSECT, true);
...
moab::ErrorCode MakeWatertightTest::move_vert(moab::EntityHandle vertex,
...
moab::ErrorCode MakeWatertightTest::rand_vert_move(moab::EntityHandle vertex,
  ...
  // set the change in the vertex to be something slightly less than the facet
  // tolerance
  dx *= tol * 0.9;
  dy *= tol * 0.9;
  dz *= tol * 0.9;
...
moab::ErrorCode MakeWatertightTest::single_vert_bump(moab::Range verts,
                                                     double bump_dist_x,
                                                     double bump_dist_y,
                                                     double bump_dist_z,
                                                     bool verbose) {
...
moab::ErrorCode MakeWatertightTest::locked_pair_bump(moab::Range verts,
                                                     double bump_dist_x,
                                                     double bump_dist_y,
                                                     double bump_dist_z,
                                                     bool verbose) {
...
moab::ErrorCode MakeWatertightTest::locked_pair_bump_rand(moab::Range verts,
                                                          double facet_tol,
                                                          bool verbose) {
  moab::EntityHandle vertex1 = verts.back();
  moab::EntityHandle vertex2 = (verts.back() - 1);
...
moab::ErrorCode MakeWatertightTest::rand_locked_pair_bump(moab::Range verts,
                                                          double bump_dist_x,
                                                          double bump_dist_y,
                                                          double bump_dist_z,
                                                          bool verbose) {
...
moab::ErrorCode MakeWatertightTest::rand_locked_pair_bump_rand(
    moab::Range verts, double facet_tol, bool verbose) {
  // select random verticies from verts
  int number_of_verts = verts.size();
  double num = rand() / static_cast<double>(RAND_MAX);
...
moab::ErrorCode MakeWatertightTest::rand_vert_bump(moab::Range verts,
                                                   double facet_tol,
                                                   bool verbose) {
  moab::EntityHandle vertex = verts.back();
  // move the desired vertex by the allotted distance
...
/// in x, y, and z
moab::ErrorCode MakeWatertightTest::adjplone_locked_pair_bump(
    moab::Range verts, double bump_dist_x, double bump_dist_y,
    double bump_dist_z, bool verbose) {
  moab::EntityHandle vertex1 = verts.back();
...
moab::ErrorCode MakeWatertightTest::adjplone_locked_pair_bump_rand(
    moab::Range verts, double facet_tol, bool verbose) {
  moab::EntityHandle vertex1 = verts.back();
  moab::EntityHandle vertex2 = (verts.back() - 2);
...
moab::ErrorCode MakeWatertightTest::nonadj_locked_pair_bump(moab::Range verts,
                                                            double bump_dist_x,
                                                            double bump_dist_y,
                                                            double bump_dist_z,
                                                            bool verbose) {
...
moab::ErrorCode MakeWatertightTest::nonadj_locked_pair_bump_rand(
    moab::Range verts, double facet_tol, bool verbose) {
  // select random verticies from verts
  int number_of_verts = verts.size();
  double num = rand() / static_cast<double>(RAND_MAX);
...
bool MakeWatertightTest::seal_and_check(moab::EntityHandle input_set,
                                        double facet_tolerance, bool verbose) {
  ...
  result = cw->check_mesh_for_watertightness(input_set, facet_tolerance, sealed,
                                             test, verbose, topo_test);
...
moab::ErrorCode MakeWatertightCylinderTest::move_vert_theta(
    moab::EntityHandle vertex, double tolerance, bool verbose) {
...
moab::ErrorCode MakeWatertightCylinderTest::move_vert_R(
    moab::EntityHandle vertex, double tol, bool verbose) {
  // get vertex coordinates
  double coords[3];
  result = MBI()->get_coords(&vertex, 1, coords);
...
moab::ErrorCode MakeWatertightCylinderTest::single_vert_bump_R(
    moab::Range verts, double facet_tol, bool verbose) {
  moab::EntityHandle vertex = verts.back();
  // move the desired vertex by the allotted distance
...
moab::ErrorCode MakeWatertightCylinderTest::rand_locked_pair_bump_theta(
    moab::Range verts, double facet_tol, bool verbose) {
  // select random verticies from verts
  int number_of_verts = verts.size();
  double num = rand() / static_cast<double>(RAND_MAX);
...
moab::ErrorCode MakeWatertightCylinderTest::theta_vert_bump(
    moab::Range verts, double bump_dist_theta, bool verbose) {
  // get vertex coordinates
  double coords[3];
  moab::EntityHandle vertex = verts.back();
  result = MBI()->get_coords(&vertex, 1, coords);
  ...
  // write new coordinates to the mesh
  // might not be necesarry any longer as we move to doing tests on a
  // moab-instance basis
  result = MBI()->set_coords(&vertex, 1, coords);
  MB_CHK_SET_ERR(result, "could not set the vertex coordinates");
...
/// tolerance
moab::ErrorCode MakeWatertightCylinderTest::locked_pair_bump_theta(
    moab::Range verts, double tolerance, bool verbose) {
  // get vertex coordinates
  moab::EntityHandle vertex1 = verts.back();
  moab::EntityHandle vertex2 = verts.back() - 1;
...
moab::ErrorCode MakeWatertightCylinderTest::adjplone_locked_pair_bump_theta(
    moab::Range verts, double facet_tol, bool verbose) {
  moab::EntityHandle vertex1 = verts.back();
  moab::EntityHandle vertex2 = (verts.back() - 2);
...
moab::ErrorCode MakeWatertightCylinderTest::locked_pair_bump_R(
    moab::Range verts, double facet_tol, bool verbose) {
  moab::EntityHandle vertex1 = verts.back();
  moab::EntityHandle vertex2 = (verts.back() - 1);
...
/// selects random verticies from verts and moves them in R a distance equal to
/// the faceting tolerance
moab::ErrorCode MakeWatertightCylinderTest::rand_locked_pair_bump_R(
    moab::Range verts, double facet_tol, bool verbose) {
  // select random verticies from verts
  int number_of_verts = verts.size();
  double num = rand() / static_cast<double>(RAND_MAX);
...
moab::ErrorCode MakeWatertightCylinderTest::adjplone_locked_pair_bump_R(
    moab::Range verts, double facet_tol, bool verbose) {
  moab::EntityHandle vertex1 = verts.back();
  moab::EntityHandle vertex2 = (verts.back() - 2);
...
moab::ErrorCode MakeWatertightCylinderTest::nonadj_locked_pair_bump_R(
    moab::Range verts, double facet_tol, bool verbose) {
  // select random verticies from verts
  int number_of_verts = verts.size();
  double num = rand() / static_cast<double>(RAND_MAX);
...
moab::ErrorCode MakeWatertightCylinderTest::nonadj_locked_pair_bump_theta(
    moab::Range verts, double facet_tol, bool verbose) {
  // select random verticies from verts
  int number_of_verts = verts.size();
  double num = rand() / static_cast<double>(RAND_MAX);
