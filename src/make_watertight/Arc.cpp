...
// Degenerate edges (same topological endpts) are caused by a prior step in
// which coincident verts are merged.
moab::ErrorCode Arc::remove_degenerate_edges(moab::Range& edges,
  ...
  while (i != edges.end()) {
    ...
    if (2 == n_verts && endpts[0] != endpts[1]) {
      ++i;
    } else if ((2 == n_verts && endpts[0] == endpts[1]) || (1 == n_verts)) {
      if (debug) {
        std::cout
            << "remove_degenerate_edges: deleting degenerate edge and tris "
            << std::endl;
      }
      ...
      std::cout << "remove_degenerate_edge: wrong edge connectivity size"
                << std::endl;
...
moab::ErrorCode Arc::remove_opposite_pairs_of_edges(moab::Range& edges,
  ...
  for (unsigned int i = 0; i < edges.size(); i++) {
    ...
    if (moab::MB_SUCCESS != result) {
      std::cout << "result=" << result << " could not get adjacencies of edge"
                << std::endl;
      return result;
    }
    ...
    result = MBI()->get_adjacencies(two_verts, 1, false, adj_edges,
                                    moab::Interface::INTERSECT);
    assert(moab::MB_SUCCESS == result);

    // if any other edges exist, they are opposite the original edge and should
  ...
  for (unsigned i = 1; i < n_orig_edges; ++i) {
    ...
    if (my_edges[j].v0 == my_edges[i].v0 && my_edges[j].v1 == my_edges[i].v1) {
      ...
      while (my_edges[j].v0 == my_edges[i].v0 &&
             my_edges[j].v1 == my_edges[i].v1 && i < n_orig_edges) {
      ...
      if (debug) {
        std::cout << "remove_opposite_edges: deleting "
                  << duplicate_edges.size() << " edges" << std::endl;
      }
...
moab::ErrorCode Arc::get_next_oriented_edge(const moab::Range edges,
  ...
  for (moab::Range::iterator i = adj_edges.begin(); i != adj_edges.end(); i++) {
    ...
    if (moab::MB_SUCCESS != result) {
      MBI()->list_entity(*i);
      std::cout << "result=" << result << " could not get connectivity of edge"
                << std::endl;
      return result;
    }
    assert(moab::MB_SUCCESS == result);
...
moab::ErrorCode Arc::create_loops_from_oriented_edges(
    moab::Range edges,
  ...
  while (0 != edges.size()) {
    ...
    // point. If it is a pinch point the loop is ambiguous. Maybe--see
    // watertightness notes for 20091201
    {
      const moab::EntityHandle* end_verts;
      int n_verts;
      result = MBI()->get_connectivity(edge, end_verts, n_verts);
      ...
      if (2 != adj_edges.size() && debug) {
        std::cout << "  create_loops: adj_edges.size()=" << adj_edges.size()
                  << std::endl;
    ...
    result =
        get_next_oriented_edge(first_edge, loop_of_edges.back(), next_edge);
    assert(moab::MB_SUCCESS == result);
...
moab::ErrorCode Arc::order_verts_by_edge(
...
moab::ErrorCode Arc::merge_curves(moab::Range curve_sets,
                                  const double facet_tol, moab::Tag id_tag,
                                  moab::Tag merge_tag, const bool debug) {
  ...
  // set tree options
  const char settings[] =
      "MAX_PER_LEAF=1;SPLITS_PER_DIR=1;PLANE_SET=0;MESHSET_FLAGS=0x1;TAG_NAME="
      "0";
  ...
  for (moab::Range::iterator i = curve_sets.begin(); i != curve_sets.end();
    ...
    moab::EntityHandle endpts[2] = {curve_i_verts.front(),
                                    curve_i_verts.back()};
    ...
    for (unsigned int j = 0; j < 2; j++) {
      ...
      assert(moab::MB_SUCCESS == result);
      // takes all leaves of the tree within a distance (facet_tol) of the
      // coordinates passed in by endpt_coords.array() and returns them in
      // leaves
      result =
          kdtree.distance_search(endpt_coords.array(), facet_tol, leaves, root);
      assert(moab::MB_SUCCESS == result);
      for (unsigned int k = 0; k < leaves.size(); k++) {
        ...
        result =
            MBI()->get_entities_by_type(leaves[k], moab::MBVERTEX, leaf_verts);
        assert(moab::MB_SUCCESS == result);
        for (unsigned int l = 0; l < leaf_verts.size(); l++) {
          double sqr_dist;
          result = gen->squared_dist_between_verts(endpts[j], leaf_verts[l],
                                                   sqr_dist);
          assert(moab::MB_SUCCESS == result);
          ...
          if (SQR_TOL >= sqr_dist) {
            ...
            // get the curves for all vertices that are within the squared
            // distance of each other
            result = MBI()->get_adjacencies(&leaf_verts[l], 1, 4, false,
                                            temp_curves);
            assert(moab::MB_SUCCESS == result);
    ...
    // now find curves who's interior vertices are also coincident and merge
    // them
    for (moab::Range::iterator j = candidate_curves.begin();
         j != candidate_curves.end(); j++) {
      ...
      if (debug) {
        std::cout << "curve i_id=" << i_id << " j_id=" << j_id
                  << " leng0=" << curve_length << " leng1=" << j_curve_length
                  << std::endl;
      }
      ...
      bool reversed;
      if (gen->dist_between_verts(curve_i_verts.front(),
                                  curve_j_verts.front()) >
          gen->dist_between_verts(curve_i_verts.front(),
                                  curve_j_verts.back())) {
      ...
      double dist;
      result =
          gen->dist_between_arcs(debug, curve_i_verts, curve_j_verts, dist);
      assert(moab::MB_SUCCESS == result);
      ...
      // Merge the endpts of the curve to preserve topology. Merging (and
      // deleting) the endpoints will also remove them from the KDtree so that
      // the merged curve cannot be selected again. Update the curves when
      // merging to avoid stale info.
      ...
      if (curve_i_verts.back() != curve_j_verts.back()) {
        result = zip->merge_verts(curve_i_verts.back(), curve_j_verts.back(),
                                  curve_i_verts, curve_j_verts);
        if (moab::MB_SUCCESS != result)
          std::cerr << "Failed to merge vertices with error: " << result
                    << std::endl;
        assert(moab::MB_SUCCESS == result);
      }
      ...
      // reverse the curve-surf senses if the merged curve was found to be
      // opposite the curve we keep
      if (reversed) {
        ...
        for (unsigned k = 0; k < surfs.size(); ++k) {
          // forward to reverse
          if (moab::SENSE_FORWARD == senses[k]) senses[k] = moab::SENSE_REVERSE;
          // reverse to forward
