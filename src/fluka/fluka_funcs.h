...
struct particle_state {
  ...
  moab::EntityHandle
      PrevRegion;  // the integer region that the particle was in previously
  int stack_count;
};
...
void flabrt(const char* function, const char* message, int function_strlen,
            int message_strlen);
 ...
 */
void g_step(double& pSx, double& pSy, double& pSz, double* pV, int& oldReg,
            const int& oldLttc, double& propStep, int& nascFlag,
            double& retStep, int& newReg, double& saf, int& newLttc,
            int& LttcFlag, double* sLt, int* jrLt);
...
/**
 * \brief In our case this functions sets newReg and newLttc to 0, and flagErr
 * to -1
 *
 * \param[in] pSx the particle x coordinate (cm)
 * \param[in] pSy the particle y coordinate (cm)
 ...
 * \param[out] newLttc used to indicate the new lattice id number
 */
void lkfxwr(double& pSx, double& pSy, double& pSz, double* pV,
            const int& oldReg, int& oldLttc, int& newReg, int& flagErr,
            int& newLttc);
...
void fldwr(const double& pX, const double& pY, const double& pZ, double& cosBx,
           double& cosBy, double& cosBz, double& Bmag, int& reg,
           int& idiscflag);
 ...
 * \param[out] *norml the normal vector to the surface, normal should always
 point towards oldReg. i.e. the inward pointing normal
...
void f_normal(double& pSx, double& pSy, double& pSz, double& pVx, double& pVy,
              double& pVz, double* norml, const int& oldReg, const int& newReg,
              int& flagErr);
...
/**
 * \brief Used to translate fluka region number ino names, in FluDAG this
 * function calls region2name, and takes the Fluka ID number and turns it into a
 * string
 *
 ...
 *        raises an error call, dumps simulation data and writes messages to
 * file
 *
 * \param[in] function_name, char* denoting the name of the function that raised
 * the error \param[in] message, char* denoting the error message you want to
 * pass \param[in] error_code, int, the error code you are raising from DAGMC
 */
void fludag_abort(const char* function_name, const char* message,
                  int error_code);
...
/**
 * \brief normal is the testable wrapper to f_normal, the external interface to
 *FLuka. it expects to be only called in certain circumstances, it expects to be
 *only called when the position is on a boundary, and that boundary belongs to
 *curRegion.
 *
...
/**
 * \brief Tests if a particle is on the boundary of the volume vol, returns 1
 *true or 0 false, expects to use the global particle state variable
 *state.next_surf, so must be called after a succesful ray fire, like g_fire
 *
...
/**
 * \brief turns an integer volume index into its string form with a period
 * appended i.e turns int 1 into "1." \param[in] volindex any integer
 ...
 *  This function writes out a simple numerical material assignment to the named
 * argument file Example usage:  mainFludag dagmc.html Outputs mat.inp  contains
 * MATERIAL and ASSIGNMAt records for the input geometry. The MATERIAL is gotten
 * by parsing the Cubit volume name on underscores. The string after "M_" is
 * considered to be the material for that volume. There are no MATERIAL cards
 * for the materials in the FLUKA_mat_set list For the remaining materials,
 * there is one MATERIAL card apiece (no dups) User-named (not predefined)
 * materials are TRUNCATED to 8 chars. User-named material id's start at 25 and
 * increment by 1 for each MATERIAL card index-id.txt  Map of FluDAG volume
 * index vs Cubit volume ids, for info only. Note that a preprocessing step to
 * this call sets up the the DAG object that contains all the geometry
 * information contained in dagmc.html. the name of the (currently hardcoded)
 * output file is "mat.inp" The graveyard is assumed to be the last region.
 *  Overall function call; calls other fludag functions
 */
void fludag_write(std::string matfile, std::string lfname);

/**
 * \brief makes a standard set of nuclides which we would like to retain in
 * nucid form
 */
...
/**
 * \brief writes all the material compisitions from the map of materials to
 * output stream
 *
...
/**
 * \brief Writes all the PyNE tally objects out that are contained in the tally
 * map
 *
 * \param[out] mstr the output stream to which the tallies are to be printed
 * \param[in] tally_map the map of all tallies in the problem indexed by the
 * tally name
 */
...
/**
 * \brief get all properties (metadata) stored on all entities of a given
 * dimension.
 *
