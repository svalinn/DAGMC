...
// CONSTRUCTOR
//---------------------------------------------------------------------------//
MeshTally::MeshTally(const TallyInput& input) : Tally(input) {
...
moab::ErrorCode MeshTally::setup_tags(moab::Interface* mbi,
  ...
  rval =
      mbi->tag_get_handle(tag_name.c_str(), num_bins, moab::MB_TYPE_DOUBLE,
                          tally_tag, moab::MB_TAG_DENSE | moab::MB_TAG_CREAT);
  MB_CHK_SET_ERR(rval, "Failed to get the tag handle");

  rval = mbi->tag_get_handle(error_tag_name.c_str(), num_bins,
                             moab::MB_TYPE_DOUBLE, error_tag,
                             moab::MB_TAG_DENSE | moab::MB_TAG_CREAT);
  MB_CHK_SET_ERR(rval, "Failed to get the tag handle");

  // create single tag to store the total
  std::string total_tally_tag_name = tag_name + "_TOTAL";
  rval = mbi->tag_get_handle(total_tally_tag_name.c_str(), tag_size,
                             moab::MB_TYPE_DOUBLE, total_tally_tag,
                             moab::MB_TAG_DENSE | moab::MB_TAG_CREAT);
  MB_CHK_SET_ERR(rval, "Failed to get the tag handle");

  std::string total_error_tag_name = error_tag_name + "_TOTAL";
  rval = mbi->tag_get_handle(total_error_tag_name.c_str(), tag_size,
                             moab::MB_TYPE_DOUBLE, total_error_tag,
                             moab::MB_TAG_DENSE | moab::MB_TAG_CREAT);
  MB_CHK_SET_ERR(rval, "Failed to get the tag handle");
