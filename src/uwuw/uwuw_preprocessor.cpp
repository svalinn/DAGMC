...
// constructor
uwuw_preprocessor::uwuw_preprocessor(std::string material_library_filename,
  ...
  // load the materials
  material_library =
      mat_lib.load_pyne_materials(material_library_filename, matlib_hdf5_path);
...
pyne::Material uwuw_preprocessor::create_new_material(pyne::Material material,
                                                      std::string density) {
  pyne::Material new_mat;  // to return
  pyne::comp_map comp = material.comp;

  // make sure to bring metadata with us
  new_mat = pyne::Material(comp, 1.0, material.density,
                           material.atoms_per_molecule, material.metadata);

  // use the name concatenator to make the fluka name
  std::string fluka_name =
      ncr->make_name_8bytes(material.metadata["name"].asString());
  ...
  if (verbose) {
    std::cout << "Making new material with name      : " << material_name
              << std::endl;
    std::cout << "                    with fluka_name: " << fluka_name
              << std::endl;
  }
...
void uwuw_preprocessor::process_materials() {
  ...
  std::map<moab::EntityHandle, std::string> volume_property_map =
      dmd->volume_material_property_data_eh;
  // loop over the volume property map

  for (it = volume_property_map.begin(); it != volume_property_map.end();
       ++it) {
  ...
  if (verbose) {
    ...
    for (m_it = material_library.begin(); m_it != material_library.end();
         ++m_it) {
      std::cout << m_it->first << ", ";
    }
    std::cout << std::endl;
  ...
  for (s_it = material_names.begin(); s_it != material_names.end(); ++s_it) {
    ...
    if (found_grave == std::string::npos && found_vacuum == std::string::npos) {
      if (material_library.count(mat_name) == 0) {
        std::cout << "material " << mat_name
                  << " was not found in the material library" << std::endl;
        exit(EXIT_FAILURE);
      }
      ...
      pyne::Material new_material = create_new_material(found_mat, density);

      // Renumbering the material will ensure uniqueness of material number for
      // each mat/density combination in the library
...
void uwuw_preprocessor::process_tallies() {
  ...
  for (t_it = tally_list.begin(); t_it != tally_list.end(); ++t_it) {
    ...
    pyne::Tally new_tally =
        pyne::Tally(tally_str.tally_type, tally_str.particle_name,
                    tally_str.entity_id, tally_str.entity_type,
                    "",                    // entity name
...
void uwuw_preprocessor::check_material_props(
    std::vector<std::string> material_props,
    std::vector<std::string> density_props, int cellid) {
  if (material_props.size() == 0) {
    if (verbose || fatal) {
      std::cout << "Volume " << cellid << " has no 'mat:' property "
                << std::endl;
      std::cout << "Please check your material assignments " << cellid
                << std::endl;
    }
  ...
  if (material_props.size() > 1) {
    if (verbose || fatal) {
      std::cout << "More than one material for volume with id " << cellid
                << std::endl;
      std::cout << cellid << " has the following material assignments"
                << std::endl;
  ...
  if (density_props.size() > 1) {
    if (verbose || fatal) {
      std::cout << "More than one density specified for " << cellid
                << std::endl;
      std::cout << cellid << " has the following density assignments"
                << std::endl;
      for (size_t j = 0; j < density_props.size(); j++) {
        std::cout << density_props[j] << std::endl;
      }
      std::cout << "Please check your density assignments " << cellid
                << std::endl;
    }
    if (fatal) exit(EXIT_FAILURE);
...
void uwuw_preprocessor::print_summary() {
  ...
  std::cout << "+----------------------------------------------------"
            << std::endl;
  std::cout << "| Volumes with multiple properties " << multiple_props.size()
            << std::endl;
  property_vector(multiple_props);
  std::cout << "+----------------------------------------------------"
            << std::endl;
  std::cout << "| Volumes with blank properties " << blank_props.size()
            << std::endl;
  property_vector(blank_props);
  std::cout << "+----------------------------------------------------"
            << std::endl;
  std::cout << "| Volumes with multiple densities " << multiple_densities.size()
            << std::endl;
...
tally_info uwuw_preprocessor::make_tally_groupname(std::string tally_props,
  ...
  // tally props should be of the form Neutron and Flux or, Photon and Current
  // etc
  ...
  if (position != std::string::npos) {
    ...
    tally_type =
        tally_props.substr(position + delimiter.length(), tally_props.length());
  }
  ...
  std::transform(tally_name.begin(), tally_name.end(), tally_name.begin(),
                 toupper);
...
void uwuw_preprocessor::check_tally_props(std::string particle_name,
                                          std::string tally_type, int dimension,
                                          int entityid) {
...
std::string name_concatenator::extract_alpha_num(std::string name) {
  ...
  for (it = name.begin(); it != name.end(); ++it) {
    char val = *it;
    int ascii = int(val);
    if ((ascii >= 65 && ascii < 91) || (ascii >= 48 && ascii < 58)) str += *it;
  }
  name = str;
  return name;
}
...
std::string name_concatenator::shift_and_increment(std::string name) {
  int count = 0;
  // the following statements in braces are intended to turn the 8 char long
  // string version of the name into a unique 8 character ID string, mainly for
  // fluka but has other utility in other codes it should turn "Special Steel"
  // into "SPECIALS" just by truncating and capitalising however, on the second
  // occurence of this it should turn the string into SPECIAL1, then SPECIAL2,
  // all until SPECI999 But, if the string is less than 8 chars anyway we should
  // use the space first, i.e. "Air, Dry N" should become "AIRDRYN ", then
  // "AIRDRYN1", then "AIRDRY99" etc

  // if the name is unique just use it
  if (used_b8_names.count(name) == 0) return name;

  // if the name isn't unique we will be appending and integer to the end
  // however we should preserve the name as much as possible, so if there are
  // any whitespace on the lhs of the string, shift left and increment there
  // instead
