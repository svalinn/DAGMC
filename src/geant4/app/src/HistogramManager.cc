...
HistogramManager::~HistogramManager() { return; }
...
std::vector<int> HistogramManager::get_senstitive_particles(int volume_id) {
  ...
  for (it = histogram_pdcnum_map.begin(); it != histogram_pdcnum_map.end();
       ++it) {
...
int HistogramManager::get_histogram_id(int volume_id,
...
void HistogramManager::print_histogram_collection() {
  ...
  for (it = histogram_collection.begin(); it != histogram_collection.end();
       ++it) {
    std::cout << "Volume id " << it->first << " has tally collections "
              << std::endl;
