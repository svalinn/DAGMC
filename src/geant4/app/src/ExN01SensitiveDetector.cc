...
ExN01SensitiveDetector::ExN01SensitiveDetector(const G4String& name,
  ...
  std::vector<G4int> sensitive_particles =
      HM->get_senstitive_particles(DetectorIndex);
...
void ExN01SensitiveDetector::Initialize(G4HCofThisEvent* hce) {
  ...
  fHitsCollection =
      new ExN01DetectorHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool ExN01SensitiveDetector::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  // TrackLike Scoring
  ...
  for (G4int i = 0; i < nofHits; i++) {
    ...
    if (hist_part_map.count(pdg) > 0) {
      ...
      score = (*fHitsCollection)[i]->GetWeight() *
              (*fHitsCollection)[i]->GetTrackLength() * cm / (DetectorVolume);
