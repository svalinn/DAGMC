...
G4VPhysicalVolume* ExN01DetectorConstruction::Construct() {
  ...
  if (rval != moab::MB_SUCCESS) {
    G4cout << "ERROR: Failed to load the DAGMC file " << uwuw_filename
           << G4endl;
    exit(1);
  }
...
void ExN01DetectorConstruction::ConstructSDandField() {
  ...
  if (workflow_data->tally_library.size() == 0) {
    ...
    std::cout << "Tallies not found, transport will happen, but no scores"
              << std::endl;
    return;
  }
  ...
  for (t_it = tally_library.begin(); t_it != tally_library.end(); ++t_it) {
    ...
    if (scorer.tally_type.find("Flux") != std::string::npos &&
      ...
      if( 0 < particle_filters.count(scorer.particle_name))
        {
          G4SDParticleFilter *filter = new
      G4SDParticleFilter(scorer.particle_name);
  ...
  for (it = volume_part_map.begin(); it != volume_part_map.end(); ++it) {
    ...
    for (str = particle_types.begin(); str != particle_types.end(); ++str) {
      ...
      if (pdc == 0) pdc = pyne::nucname::id(particle_name);
    ...
    // get detector volume
    double det_volume =
        dag_logical_volumes[vol_idx]->GetSolid()->GetCubicVolume();

    // create new detector
    G4VSensitiveDetector* detector = new ExN01SensitiveDetector(
        detector_name, "flux", sd_index, det_volume * cm3, HM);
...
/* add histogram for */
void ExN01DetectorConstruction::add_histogram_description(
...
void ExN01DetectorConstruction::BuildParticleFilter(
