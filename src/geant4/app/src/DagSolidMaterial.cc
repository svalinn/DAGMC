...
std::map<std::string, G4Material*> load_uwuw_materials(UWUW* workflow_data) {
  ...
  if (material_library.size() == 0) return g4_mat_empty;
...
// Given a pyne material library, get hold of the materials and return the
// collection of nuclides in the problem *note* using the name istope is not
// correct but follows the Geant4 style
std::map<int, G4Isotope*> get_g4isotopes(
    pyne::MaterialLibrary material_library) {
  ...
  // loop over the nuclides in the composition, making a new g4isotope for each
  // one
  for (auto nuc_id : nuclide_list) {
    G4Isotope* new_iso = new G4Isotope(
        name = pyne::nucname::name(nuc_id), iz = pyne::nucname::znum(nuc_id),
        n = pyne::nucname::anum(nuc_id),
...
std::map<int, G4Element*> get_g4elements(
    std::map<int, G4Isotope*> isotope_map) {
  ...
  for (it = isotope_map.begin(); it != isotope_map.end(); ++it) {
    G4Element* tmp =
        new G4Element(name = pyne::nucname::name(it->first),
                      symbol = pyne::nucname::name(it->first), ncomponents = 1);
...
std::map<std::string, G4Material*> get_g4materials(
    std::map<int, G4Element*> element_map,
    pyne::MaterialLibrary material_library) {
  ...
  // Add vacuum
  double z, a, density;
  G4Material* Vacuum = new G4Material("mat:Vacuum", z = 1., a = 1.0 * g / mole,
                                      density = 1.0e-20 * mg / cm3);
  // add to lib
