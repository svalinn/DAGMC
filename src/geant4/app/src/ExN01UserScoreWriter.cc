...
void ExN01UserScoreWriter::DumpAllQuantitiesToFile(const G4String& fileName,
  ...
  if (fileName.substr(fileName.length() - 4) != ".h5m") file_name += ".h5m";
  ...
  int extents[6] = {0, 0, 0, num_bins[0], num_bins[1], num_bins[2]};

  G4ThreeVector size;
  size = fScoringMesh->GetSize();

  std::vector<double> x_vals =
      generate_bin_bounds(num_bins[0], size.getX() / cm);
  std::vector<double> y_vals =
      generate_bin_bounds(num_bins[1], size.getY() / cm);
  std::vector<double> z_vals =
      generate_bin_bounds(num_bins[2], size.getZ() / cm);
  ...
  for (it = scMap.begin(); it != scMap.end(); ++it) {
    ...
    // create a tag
    moab::Tag tag_handle;
    rval = MBI()->tag_get_handle(score_name.c_str(), 1, moab::MB_TYPE_DOUBLE,
                                 tag_handle,
...
// num_bound bins
std::vector<double> ExN01UserScoreWriter::generate_bin_bounds(
    int num_bounds, double end_coord) {
...
moab::ErrorCode ExN01UserScoreWriter::generate_moab_mesh(
  ...
  for (int i = 0; i < num_x - 1; i++) {
    for (int j = 0; j < num_y - 1; j++) {
      for (int k = 0; k < num_z - 1; k++) {
        ...
        connect[2] = verts[k + (j * num_z) + (i * num_z * num_y) +
                           (num_z * num_y) + num_z];
        connect[3] =
            verts[k + (j * num_z) + (i * num_z * num_y) + (num_z * num_y)];
        connect[4] = verts[k + (j * num_z) + (i * num_z * num_y) + 1];
        connect[5] = verts[k + (j * num_z) + (i * num_z * num_y) + num_z + 1];
        connect[6] = verts[k + (j * num_z) + (i * num_z * num_y) +
                           (num_z * num_y) + num_z + 1];
        connect[7] =
            verts[k + (j * num_z) + (i * num_z * num_y) + (num_z * num_y) + 1];
