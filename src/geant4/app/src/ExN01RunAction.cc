...
void ExN01RunAction::EndOfRunAction(const G4Run* run) {
  ...
  if ( analysisManager->GetH1(1) ) {
   ...
   for ( it = workflow_data.tally_library.begin() ; it !=
  workflow_data.tally_library.end() ; ++it )
    {
  ...
  ss.str()+"/"+(it->second).tally_type+"/"+(it->second).particle_name;
      ...
      G4cout << tally_name+"  : mean = " <<
  analysisManager->GetH1(index)->mean()*cm*cm
                            << " rms = " <<
  analysisManager->GetH1(index)->rms()*cm*cm  << G4endl;
  ...
  */
  ...
  workflow_data.tally_library.end() ; ++it )
  {
    ...
    // get the tally name
    std::string tally_name =
  ss.str()+"_"+(it->second).tally_type+"_"+(it->second).particle_name; std::cout
  << tally_name << std::endl;
  ...
  */
  ...
  std::map<std::string, pyne::Tally>::iterator it;
  // loop over histograms and get data
  for (it = workflow_data->tally_library.begin();
       it != workflow_data->tally_library.end(); ++it) {
    int index = 1 + std::distance(workflow_data->tally_library.begin(), it);
