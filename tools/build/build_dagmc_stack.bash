...
do
    ...
    --no_fluka)		no_fluka="yes";;	
    --fluka_source)	fluka_source=$2 ; shift;;
...
Usage:    build_dagmc_stack.bash [OPTIONS]
        ...
        automatic building of FluDAG stack.
        Builds cgm, hdf5, MOAB and Fluka from svn download (default for
...
OPTIONS:
    ...
    --no_fluka                  Do not build Fluka
    --fluka_source=SRC          FLUKA Source Code located in SRC ["'$FLUPRO'"]      
"     
    exit
fi
...
if [ "$no_fluka" != "" ]
then
    echo "Quitting without building fluka"
	exit
fi
...
cp $prefix/DAGMC/FluDAG/src/rfluka $FLUPRO/flutil/.

cd $dagmc_prefix/FluDAG
mkdir bld
cd bld
cmake ../src/. -DMOAB_HOME=$moab_prefix
make
