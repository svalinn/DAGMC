...
tally assignments from the MOAB geometry in the DAGMC .h5m file. In particular,
the mapping of material definitions to geometric EntitySets generated by the
DAGMC metadata class is directly transferred to the UWUW class in the
``process_materials`` method. Certain utility functions of DAGMC's metadata
class like ``unpack_string`` are used in UWUW as well for convenience in other
areas of UWUW as well.
