 ...
 */
ErrorCode TrackLengthMeshTally::get_all_intersections(const CartVect& position,
            ...
            const std::vector<EntityHandle>& triangles)
{
  ...
  // where the ray ends terminates in the middle of a tet, or the ray could end 
  // in free space if the mesh is re-entrant
  double last_intersection = intersections.back();
  if ( last_intersection < event.track_length )
  {
    ...
    {
      std::cout << "Negative Track Length!!" << std::endl;
      std::cout << track_length << " " << last_intersection << " " <<  event.track_length << std::endl;
      std::cout << tet << " " << next_tet << std::endl;
    }
